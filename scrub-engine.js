...
function mountScrollWorld(container, config) {
  ...
  const nav = el('nav', 'sw-nav'); nav.setAttribute('aria-label', '场景导航'); if (config.nav !== false) topbar.appendChild(nav);
  ...
  const route = el('div', 'sw-route'); route.setAttribute('role', 'navigation'); route.setAttribute('aria-label', '场景进度');
  ...
  SECTIONS.forEach((s, i) => {
    ...
    dot.setAttribute('aria-label', `跳转至${s.label || `场景 ${i + 1}`}`);
    dot.innerHTML = `<span class="sw-route__label">${esc(s.label || '')}</span><i aria-hidden="true"></i>`;
  ...
  function read() {
    ...
    if (near !== activeIndex) {
      ...
      nav.querySelectorAll('.sw-nav__item').forEach((n, k) => { n.classList.toggle('is-active', k === near); n.setAttribute('aria-current', k === near ? 'true' : 'false'); });
...
function injectCSS() {
  ...
  .sw-brand:focus-visible,.sw-nav__item:focus-visible,.sw-route__dot:focus-visible,.sw-btn:focus-visible{outline:3px solid var(--sw-accent);outline-offset:3px;}
  ...
  .sw-route__dot{position:relative;border:0;background:transparent;cursor:pointer;width:28px;height:28px;display:grid;place-items:center;}
